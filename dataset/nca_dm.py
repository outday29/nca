...
class NCADatasetBase(Dataset):
  ...
  def get_random_seed_cache(self):
    ...
    cached_seed = list(filter(lambda x: x.is_file() and str(x).endswith(".pt"), cached_seed))
  ...
  def get_alive_mask(self, x):
      ...
      return (
          F.max_pool2d(
                  x[:, self.alive_channel_idx: self.alive_channel_idx + 1, :, :], # To have the unsqueeze effect
                  kernel_size=3,
                  stride=1,
                  padding=1,
          )
  ...
  def __getitem__(self, idx):
    ...
    if random.random() > 0.2:
      if self.cache_is_empty():
        ...
        return (new_seed, self.target_image_processed)
      
      else:
        return (self.get_random_seed_cache(), self.target_image_processed)
